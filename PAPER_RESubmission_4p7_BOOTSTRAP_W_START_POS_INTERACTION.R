...
##
## save(projected.points.intercept, file= paste0("Bootstrap/NEW_NEW_RESUBMISSION_CFB_W_START_POS_INTERACTION_NOT_BALANCED_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.points.intercept.RData"))
## save(projected.by.game, file= paste0("Bootstrap/NEW_NEW_RESUBMISSION_CFB_W_START_POS_INTERACTION_NOT_BALANCED_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.by.game.RData"))
##
...
for (year in 2020){
  ...
  for (b in 1:n.boot){
    ...
    off.proj.mat[, "pos.homefieldTRUE"] <- mean(off.proj.mat[, "pos.homefieldTRUE"])
    ...
    def.proj.mat[, "pos.homefieldTRUE"] <- mean(def.proj.mat[, "pos.homefieldTRUE"])
    ...
    if (b %% 100 == 0){
      if (balanced){
        save(projected.points.intercept, file= paste0("Bootstrap/NEW_NEW_RESUBMISSION_CFB_W_START_POS_INTERACTION_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.points.intercept.RData"))
        save(projected.by.game, file= paste0("Bootstrap/NEW_NEW_RESUBMISSION_CFB_W_START_POS_INTERACTION_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.by.game.RData"))
