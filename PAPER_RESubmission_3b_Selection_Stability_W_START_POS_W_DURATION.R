...
## CFB
##
## Reproducing the selection stability Figure 6(b) from the main paper (in the appendix),
...
type <- NULL
# type <- c("ONLY_TURNOVERS","ONLY_TAKEAWAYS")[2]
...
# year.range <- year.range[!(year.range %in% c(2014,2015,2017,2019))]
...
stat.order <- c("off.ST.yards.gained", "ST.return.yards.net",
                ...
                # "turnover.nonscor", 
                # "turnover.nonscor:start.pos.post.turnover",
...
for (year in year.range){
  ...
  # load(paste0("MULT_REP_NONPARALLEL_ORDNET/RESUBMISSION_CLEANED/CFB_year=", as.character(year), "_nlambdas=", n.lambdas, "_nfolds=", n.folds, "_nreps=", n.CV.reps, "_lambda.1se.est.RData"))
  
  load(paste0("MULT_REP_NONPARALLEL_ORDNET/RESUBMISSION_CLEANED/CFB_START_POS_INTERACTION_W_DURATION_year=", as.character(year), 
              "_NO_TAKEAWAYS",
              "_stand=", stand, "_nlambdas=", n.lambdas, "_nfolds=", n.folds, "_nreps=", n.CV.reps, "_lambda.1se.est.RData"))
