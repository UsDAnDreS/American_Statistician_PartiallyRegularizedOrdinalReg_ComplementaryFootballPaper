...
## CFB
##
## Reproducing the selection stability Figure 4(a) from the supplementary materials,
## which calculates the proportion of times each complementary statistic got selected
## via lambda.1se 10-fold CV across all CFB seasons and replicates.
#####
...
## If want to exclude the takeaways, only use turnovers
NO_TAKEAWAYS <- TRUE
...
stat.order <- c("off.ST.yards.gained", "ST.return.yards.net",
                ...
                "takeaway.nonscor", "TO.nonscor", "punt.safety",
...
for (year in year.range){
  ...
  load(paste0("MULT_REP_NONPARALLEL_ORDNET/RESUBMISSION_CLEANED/CFB_W_DURATION_year=", as.character(year), ifelse(NO_TAKEAWAYS, "_NO_TAKEAWAYS",""), ifelse(stand, "_stand=TRUE", "_stand=FALSE"), "_nlambdas=", n.lambdas, "_nfolds=", n.folds, "_nreps=", n.CV.reps, "_lambda.1se.est.RData"))
...
for (j in 1:(n.levels-1)){
  ...
  for (year in year.range){
    # Loading the estimate and recording the sign and selection status for each statistic.
    # load(paste0("MULT_REP_NONPARALLEL_ORDNET/RESUBMISSION_CLEANED/CFB_year=", as.character(year), ifelse(stand, "", "_stand=FALSE"), "_nlambdas=", n.lambdas, "_nfolds=", n.folds, "_nreps=", n.CV.reps, "_lambda.1se.est.RData"))
    load(paste0("MULT_REP_NONPARALLEL_ORDNET/RESUBMISSION_CLEANED/CFB_W_DURATION_year=", as.character(year), ifelse(NO_TAKEAWAYS, "_NO_TAKEAWAYS",""), ifelse(stand, "_stand=TRUE", "_stand=FALSE"), "_nlambdas=", n.lambdas, "_nfolds=", n.folds, "_nreps=", n.CV.reps, "_lambda.1se.est.RData"))
...
rownames(lambda.1se.condensed.mat) <- str_replace(rownames(lambda.1se.condensed.mat), fixed("turnovers"), fixed("TO"))
