...
#########
## CFB, Single-Game adjustments (supplementary materials, Tables 7 & 8)
...
load("RESUBMISSION_CFB_LEAVE_CONTEXT_AS_IS_ORDNET_projected.points.intercept.RData")
...
# 
#   if (balanced){
#   # load(paste0("Bootstrap/RESUBMISSION_CFB_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", year, "_ORDNET_projected.points.intercept.RData"))
#   # load(paste0("Bootstrap/RESUBMISSION_CFB_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", year, "_ORDNET_projected.points.intercept.RData"))
#  # load(paste0("Bootstrap/NEW_RESUBMISSION_CFB_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", year, "_ORDNET_projected.by.game.RData"))
#   load(paste0("Bootstrap/NEW_RESUBMISSION_CFB_W_START_POS_INTERACTION_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", year, "_ORDNET_projected.by.game.RData"))
# 
#     } else {
#       load(paste0("Bootstrap/NEW_RESUBMISSION_CFB_W_START_POS_INTERACTION_NOT_BALANCED_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", year, "_ORDNET_projected.by.game.RData"))
# }
...
#   interm.df <- NULL
# 
#   for (b in 1:length(projected.by.game[[as.character(year)]])){
#     print(b)
...
# 
# 
# # save(file=paste0("NEW_W_START_POS_INTERACTION_my.df.balanced.by.game.",side,".Robj"),
# #      my.df)
# 
# if (balanced){
#   save(file=paste0("NEW_W_START_POS_INTERACTION_my.df.balanced.by.game.",side,".Robj"),
#        my.df)
# } else {
#   save(file=paste0("NEW_W_START_POS_INTERACTION_my.df.not.balanced.by.game.Robj"),
#        my.df)
# }
...
# load("my.df.Robj)
load(paste0("NEW_W_START_POS_INTERACTION_my.df.not.balanced.by.game.Robj"))
...
if (side == "Offense"){
  ...
  boot.df <- my.df %>%
    filter(Offense.Halves.Included > 1) %>%
  ...
  boot.df <- my.df %>%
    filter(Defense.Halves.Included > 1) %>%
    group_by(Team, Year, game_id) %>%
...
if (side == "Offense"){
  our.avg.mat <- pbp_by_drive_full %>%
    group_by(Year, def_pos_team, game_id, pos_team, pos.homefield) %>%
  ...
  our.avg.mat <- our.avg.mat %>% left_join(pbp_by_drive_full %>%
                                             group_by(Year, pos_team, game_id# , def_pos_team, pos.homefield
                                             ) %>%
                                             summarize(n.drives.played = nrow(unique(data.frame(game_id, game_id_drive))),
  ...
  our.avg.mat <- our.avg.mat %>% left_join(pbp_by_drive_full %>%
                                             group_by(Year, def_pos_team, game_id) %>%
                                             summarize(n.drives.played = nrow(unique(data.frame(game_id, game_id_drive))),
                                                       start_pos_post_turnover.per.drive = mean(lagged_start_pos_post_turnover[lagged_turnovers.nonscor == TRUE], na.rm=T)) %>%
...
if (side == "Offense"){
  ...
  our.rank.mat <- data.frame(Team=our.avg.mat$Team,
                             ...
                             game_id=factor(our.avg.mat$game_id),
