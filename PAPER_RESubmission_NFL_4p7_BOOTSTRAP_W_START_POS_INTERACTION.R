...
## NFL, 
##
## Using bootstrap replication of the following process:
##  Fit the reduced models with consistently selected complementary features,
...
## 
## The resulting expected points projections are saved in 
## "projected.points.intercept" and "projected.by.game" objects, and the following files:
##
## save(projected.points.intercept, file= paste0("Bootstrap/NEW_RESUBMISSION_NFL_W_START_POS_INTERACTION_NOT_BALANCED_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.points.intercept.RData"))
## save(projected.by.game, file= paste0("Bootstrap/NEW_RESUBMISSION_NFL_W_START_POS_INTERACTION_NOT_BALANCED_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.by.game.RData"))
##
## The bootstrap replicates are then used to generate the quantile confidence intervals for our projections.
#######
...
# Side of the ball to be balanced
side <- "Offense"
...
for (year in 2017){
  ...
  projected.by.game[[as.character(year)]] <- list()
  
  pbp_by_drive <- read.csv("NFL_2009_2017_Drive_by_Drive_Data_Cleaned.csv") %>%
    filter(Season == year)
  
  print(length(unique(pbp_by_drive$pos_team)))
  ...
  ## Need to retain the original dataset to keep bootstrapping from
  pbp_by_drive.noNA_original <- pbp_by_drive.noNA
  
  #############
  ## Need to define the actual original design matrix,
  ## because I was predicting for bootstrap samples instead of the original dataset...
  #############
  ...
  pred.mat.X.with.complem <- model.matrix(lm.obj)[,-1]
  pred.y.with.complem <- pbp_by_drive.noNA$categ.score
  ...
  pred.mat.X.no.complem <- model.matrix(lm.no.complem.obj)[,-1]
  pred.y.no.complem <- pbp_by_drive.noNA$categ.score
  ...
  for (b in 1:n.boot){
    ...
    cat("\n")
    print(paste0("Bootstrap replicate #: ", b))
    cat("\n")
    
    ## Neet to also have an object for the final bootstrap data frame
    pbp_by_drive.noNA_for_final_df <- pbp_by_drive.noNA_original
    
    ## Bootstrap resampling
    ## MAKING SURE IT'S BALANCED, with the AMOUNT OF DRIVES
    
    if (balanced){
      
    if (side == "Offense"){
      pbp_by_drive.noNA <- pbp_by_drive.noNA_original %>%
        group_by(pos_team) %>%
      ...
      pbp_by_drive.noNA <- pbp_by_drive.noNA_original[sample(1:nrow(pbp_by_drive.noNA_original), 
                                                             replace=TRUE), ]
    }
    ...
    ########
    ### CALCULATING ORDINAL NET OBJECTS (FROM "RESubmission_4_Reduced_Model")
    ########
    ...
    lm.obj <- lm(formula(paste0("as.numeric(categ.score) ~", paste0(c("pos_team", "def_pos_team", "pos.homefield", "complem.ind", 
                                                                      paste0(c(relev.var.names.lagged[1], 
                                                                               paste0(relev.var.names.lagged[1],":", relev.var.names.lagged[2])), ":complem.ind", sep="")
                                                                      , "score_diff_start", "drive_time_left_in_half", "drive_half_start"
                                                                      , "score_diff_start:drive_time_left_in_half", "score_diff_start:drive_half_start", "drive_time_left_in_half:drive_half_start"
    ),
    collapse = " + "), collapse =" ")),
    ...
    def.proj.mat <- pred.mat.X.no.complem
    ...
    off.proj.mat <- pred.mat.X.with.complem
    off.proj.mat[, str_detect(colnames(off.proj.mat), "def_pos")] <- 0
    off.proj.mat[, "pos.homefield"] <- mean(off.proj.mat[, "pos.homefield"])
    ...
    def.proj.mat <- pred.mat.X.with.complem
    def.proj.mat[, str_detect(colnames(def.proj.mat), "off_pos")] <- 0
    def.proj.mat[, "pos.homefield"] <- mean(def.proj.mat[, "pos.homefield"])
    ...
    pbp_by_drive.noNA_for_final_df <- data.frame(pbp_by_drive.noNA_for_final_df,
                                                 Boot.pts_scored_num = pbp_by_drive.noNA$pts_scored_num)
    
    ## Making sure we also keep the actual per-drive point averages
    
    pbp_by_drive.noNA_for_final_df$pts_scored_num <- ifelse(pbp_by_drive.noNA_for_final_df$categ.score == "Offensive TD", 
                                                            7,
                                                            ifelse(pbp_by_drive.noNA_for_final_df$categ.score == "Field Goal",
                                                                   3, 
                                                                   ifelse(pbp_by_drive.noNA_for_final_df$categ.score == "No Score",
                                                                          0,
                                                                          ifelse(pbp_by_drive.noNA_for_final_df$categ.score == "Safety",
                                                                                 -2,
                                                                                 -7))))
    
    projected.points.intercept[[as.character(year)]][[b]] <- pbp_by_drive.noNA_for_final_df %>%
      group_by(pos_team) %>%
      summarise(
        Offense.Avg.Points.Actual = mean(pts_scored_num),
        Offense.Avg.Points.Boot = mean(Boot.pts_scored_num),
      ...
      ) %>% left_join(pbp_by_drive.noNA_for_final_df %>% 
                        group_by(def_pos_team) %>%
                        summarise(
                          Defense.Avg.Points.Actual = mean(pts_scored_num),
                          Defense.Avg.Points.Boot = mean(Boot.pts_scored_num),
    ...
    projected.by.game[[as.character(year)]][[b]] <- pbp_by_drive.noNA_for_final_df %>%
      ...
      summarise(
        ...
        Offense.Halves.Included =length(unique(game_id_half))
                      ...
                      by = c("game_id", "pos_team" = "def_pos_team")) %>%
      rename(Team = pos_team)
    
    if (b %% 100 == 0){
      if (balanced){
        save(projected.points.intercept, file= paste0("Bootstrap/NEW_RESUBMISSION_NFL_W_START_POS_INTERACTION_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.points.intercept.RData"))
        save(projected.by.game, file= paste0("Bootstrap/NEW_RESUBMISSION_NFL_W_START_POS_INTERACTION_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.by.game.RData"))
  ...
  # save(projected.points.intercept, file= paste0("Bootstrap/RESUBMISSION_NFL_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.points.intercept.RData"))
  # save(projected.by.game, file= paste0("Bootstrap/RESUBMISSION_NFL_BALANCED_", side, "_LEAVE_CONTEXT_AS_IS_BOOTSTRAP_", as.character(year), "_ORDNET_projected.by.game.RData"))
