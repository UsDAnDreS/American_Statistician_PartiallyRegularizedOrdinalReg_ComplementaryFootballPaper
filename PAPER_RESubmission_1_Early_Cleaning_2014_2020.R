#########
### Cleaning the play-by-play CFB data obtained from "cfbfastr" R package
###   (it had A TON of issues and data inconsistencies),
### eventually converting it into one drive-by-drive dataset per season
##
## Saved into a file via the following command:
##    save(file=paste0("pbp_by_drive_", year, ".Robj"), pbp_by_drive)
##
## NOTE: You will first need to split up the ""pbp_on_campus_full_data.Robj" object
##       obtained from previous stage (of pulling from 'cfbfastr' package)
##       into separate datasets, year-by-year, titled as:
##       "pbp_[year]_on_campus_data.Robj"
## [THERE'S NO CODE SAVED FOR THAT, AS IT WAS DONE IN CONSOLE INITIALLY...]
#########
...
## Making sure halves are categorical
pbp$half <- ifelse(pbp$half == 1, "1st", "2nd")


pbp_by_drive <- pbp %>% select(game_id, drive_number, game_id_drive, game_id_half, week, home,  # drive_id,
                               play_type,  play_text,  half, period,
  ...
  summarize(n.pass_attempts = sum(pass_attempt, na.rm=T),
            ...
            drive_half_start = head(half, 1),
            drive_half_end = tail(half, 1),
            drive_qtr_start = head(period, 1),
            drive_qtr_end = tail(period, 1),
...
pbp_by_drive[our.ind, ]$score_pts <- ifelse(str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen),  fixed("kick)")),
                                            7,
                                            ifelse(str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen), fixed("two-point")) & !str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen), "failed"),
                                                   8,
                                                   6))
...
pbp_by_drive[our.ind, ]$score_pts_by_text <- ifelse(str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen),  fixed("kick)")),
                                                    7,
                                                    ifelse(str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen), fixed("two-point")) & !str_detect(tolower(pbp_by_drive[our.ind, ]$last.play.text.no.pen), "failed"),
                                                           8,
                                                           6))
...
# pass = 0/1 - includes SACKS for some reason.. in addition to actual pass attempts
# pass_attempt = 0/1 - is expected to just have actual pass attempts 
# rush = 0/1 - just rush attempts?
...
#   3rd down conversion %
#   Kicks/Punts blocked %
#   Total tackles per play
#   Tackles for loss per play
#   QB Hurries to Pass Att ratio
#   Sacks to Pass Att ratio
#   Passes Defended per play
#   Forced Fumbles per play
#   Punt/Safety %
#   Non-Scoring TO %
#   Points
#   Defensive TD %
...
##########
## QUESTIONS/TASKS:
##    1. COMPARE "yds_rushed" VS "rush_yds", "yds_receiving" VS "reception_yds", ... MAKING SURE THESE ARE THE SAME
##    2. What's  "completion_yds"????
##    3. What are all these "_stat" VARIABLES???
##########

##    1. COMPARE "yds_rushed" VS "rush_yds", "yds_receiving" VS "reception_yds", ... MAKING SURE THESE ARE THE SAME

sum(pbp$yds_rushed != pbp$rush_yds, na.rm=T)

## yds_rushed is more accurate
pbp[which(pbp$yds_rushed != pbp$rush_yds), c("rush_yds", "yds_rushed", "play_text")]

## yds_receiving is more accurate
pbp[which(pbp$yds_receiving != pbp$reception_yds), c("yds_receiving", "reception_yds", "play_text")]


##    2. What's  "completion_yds"???? 
## Evidently the yards for the completion, REGARDLESS OF WHAT HAPPENS AFTERWARDS IN THE PLAY
## (e.g. fumbles, laterals, etc etc)
## PROBABLY BETTER OFF JUST USING "YDS_RECEIVING".. who cares about PURE COMPLETION YARDAGE...
pbp[which(pbp$yds_receiving != pbp$completion_yds), c("yds_receiving", "completion_yds",  "play_text")]


##    3. What are all these "_stat" VARIABLES??? How are they different from vars with same name but NO "_stat"??

## "target" is clearly more accurate than "target_stat" - NOT SURE WHAT THIS IS... NO NEED TO USE
# pbp[which(pbp$target_stat != pbp$target), c("target_stat", "target", "play_text")]
# View(pbp[which(pbp$target_stat != 0 | pbp$target != 0), c("target_stat", "target", "play_text")])

## "interception_stat" is the same as "interception_thrown_stat", but either sometimes FAULTILY gives out NA (no tendency which one)
## NO NEED TO USE
# pbp[which(pbp$interception_stat != pbp$interception_thrown_stat), c("interception_stat", "interception_thrown_stat", "play_text")]
# View(pbp[which(pbp$interception_stat == 1), c("interception_stat", "interception_thrown_stat", "play_text")])
# View(pbp[which(pbp$interception_thrown_stat == 1), c("interception_stat", "interception_thrown_stat", "play_text")])


# "touchdown" is more accurate
# NO NEED TO USE (will use DRIVE RESULTS)
# pbp[which(pbp$touchdown_stat == pbp$touchdown), c("touchdown_stat", "touchdown", "play_text")]

# "fumble" variables... if one were to use anything, just use "fumble_vec" - the rest are pretty sketchy.
# BUT WE JUST USE TEXT RECOGNITION ("fumbl" in "play_text")
#
# pbp[which(pbp$fumble_stat != pbp$fumble_vec), c("fumble_vec", "fumble_recovered_stat", "fumble_forced_stat", "fumble_stat", "play_text")]
# pbp[which(pbp$fumble_vec == 1), c("fumble_vec", "fumble_recovered_stat", "fumble_forced_stat", "fumble_stat", "play_text")]

# "sack" & "sack_stat" variables are the exact same, so, IF ONE WERE TO USE ANYTHING, just keep "sack";  "sack_stat" is pointless
# NO NEED TO USE, using the TEXT RECOGNITION INSTEAD
# View(pbp[!is.na(pbp$sack_stat), c("sack_vec", "sack", "sack_stat", "play_text")])
# View(pbp[which(pbp$sack_vec != pbp$sack), c("sack_vec", "sack", "sack_stat", "play_text")])

## "yds_sacked" = negative yardage, "sack_taken_stat" = absolute-value yardage lost (positive)
## BUT if there's a SACK-FUMBLE, the "sack_taken_stat" CHANGES SIGN.. NOT SURE IF WE REALLY NEED THAT..
## as this play should still go as NEGATIVE for OFFENSE, POSITIVE for DEFENSE..
##  So.. just do "yds_sacked"
## => USING IT IN THE DRIVE CUMULATIONS
# View(pbp[which(pbp$yds_sacked != -pbp$sack_taken_stat), c("yds_sacked", "sack_taken_stat", "play_text")])

summary(pbp$yds_sacked)
summary(pbp$sack_taken_stat)


## !! !PASS BREAKUPS !!!
## "pass_breakup_stat" - sometimes are =1 when they SHOULD NOT BE (there's NO PASS or BREAKUP going on)
##  => so just use TEXT RECOGNITION "broken up"
##  BUT STILL KEEP THAT "pass_breakup_stat" variable around
View(pbp[which(pbp$pass_breakup_stat == 1), c("pass_breakup_stat", "play_text")])
table(pbp$pass_breakup_stat, str_detect(tolower(pbp$play_text), "broken up"))
View(pbp[which(pbp$pass_breakup_stat == 1 & !str_detect(tolower(pbp$play_text), "broken up")), c("play_type", "play_text")])


## Red zone visits
#   rz_play = 0/1 indicator is NOT fully reliable, sometimes gets carried away with a certain drive...
#   SO just create it yourself BASED ON "at least one play with yards_to_goal <= 20"
sum(pbp$yards_to_goal[pbp$rz_play == 1] > 20, na.rm=T)
head(sort(pbp$yards_to_goal[pbp$rz_play == 1], decreasing = T), 10)

## ??? TACKLES ???
##   NOPE, NO STATS FOR TACKLING


## "stuffed run"
## RUNS FOR 0 GAIN or A LOSS
##  PENALTIES evidently count as "stuffed runs"...
##  TEXT RECOGNITION IS BETTER.
View(pbp[pbp$stuffed_run == 1, c("play_type", "play_text")])
table(pbp$stuffed_run, pbp$rush == 1 & (str_detect(tolower(pbp$play_text), "no gain") | str_detect(tolower(pbp$play_text), "for a loss")) )

View(pbp[pbp$stuffed_run == 1 & !(pbp$rush == 1 & (str_detect(tolower(pbp$play_text), "no gain") | str_detect(tolower(pbp$play_text), "for a loss"))),
                                                       c("play_type", "play_text")])

View(pbp[pbp$stuffed_run == 0 & (pbp$rush == 1 & (str_detect(tolower(pbp$play_text), "no gain") | str_detect(tolower(pbp$play_text), "for a loss"))),
         c("play_type", "play_text")])


#  For INCOMPLETIONS there's a separate stat variable:
#  "incompletion_stat"

# All the PENALTY indicators should be worthy of attention:
# "penalty_flag", "penalty_declined", "penalty_no_play", "penalty_offset", "penalty_text", "penalty_play_text"

### ALL RELEVANT PLAY-VARIABLES
relev.play.vars <- c("game_id", "pos_team", "def_pos_team", "pos_team_score", "def_pos_team_score", 
                     "period", "clock.minutes", "clock.seconds",
                     "play_type", "play_text", 
                     "down", "yards_gained", "yards_to_goal", "yards_to_goal_end",
                     "penalty_detail", "yds_penalty", "penalty_1st_conv",
                     "firstD_by_kickoff", "firstD_by_poss", "firstD_by_penalty", "firstD_by_yards",
                     #"success", 
                     "rz_play", 
                     #"scoring_opp", 
                     "stuffed_run", "change_of_pos_team", "downs_turnover", "turnover", "pos_score_diff_start", "pos_score_pts",
                    # "Goal_To_Go", "home", "away", "end_of_half",
                    # "down_end", "distance_end", "yards_to_goal_end", "Goal_To_Go_end",
                     "offense_score_play", "defense_score_play", 
                     "yard_line", "scoring", 
                    "pos_score_diff", "pos_score_diff_start_end", # "change_of_poss",
                     "score_pts", "offense_score", "defense_score",
                     "yds_rushed", "yds_receiving", "yds_sacked", "yds_int_return", "yds_fumble_return", 
                     "yds_punted", "yds_punt_gained", "yds_fg", "yds_kickoff", "yds_kickoff_return",
                     "drive_result_detailed", 
                     "new_drive_pts", "drive_start_yards_to_goal", "drive_end_yards_to_goal", 
                    "drive_yards",
                     ...
                     "drive_time_minutes_elapsed", "drive_time_seconds_elapsed",
                     "pts_scored",
                     "rush", "rush_td", "pass", "pass_td", "completion", "pass_attempt", # "target", 
                     "sack_vec", "sack", "int", "int_td", "turnover_vec",
                     "scoring_play", "td_play", "touchdown", "safety", "fumble_vec", 
                     "kickoff_tb", "kickoff_onside", "kickoff_oob", "kickoff_fair_catch", "kickoff_downed", "kickoff_safety", "kick_play",
                     "punt", "punt_play", "punt_tb", "punt_oob", "punt_fair_catch", "punt_downed", "punt_safety", "punt_blocked", 
                     "penalty_safety", "fg_inds", "fg_made",
                     "team_score", "opponent_score",
                     "rush_yds", "reception_yds", "completion_yds",
                     
                     "interception_stat", "incompletion_stat", 
                     # "target_stat", "interception_thrown_stat", "touchdown_stat"
                     # "fumble_recovered_stat", "fumble_forced_stat", "fumble_stat", 
                     "sack_stat", # "sack_taken_stat",
                     "pass_breakup_stat",
...
## 23 different outcomes. Let's get 2 RANDOM DRIVES per EACH OUTCOME
unique(pbp_by_drive$drive_result_detailed)

set.seed(1)

## To check SEPARATELY -
##    DRIVE RESULTS: 
##        "Fumble", "Fumble Recovery (Opponent) Touchdown", 
##        "Punt Team Fumble Lost",  "Interception Return",
##        "Kickoff Team Fumble Recovery", "Blocked Field Goal", "Fumble Return Touchdown",
##        "Punt Return TD", "Interception Return Touchdown", "Safety", "Blocked Punt TD",
##        "Punt Team Fumble Lost Touchdown",  "Kickoff Return Touchdown",  "Blocked Field Goal Touchdown",
##        "Missed Field Goal Return"
##    PLAY TYPE: 
##        "Fumble Recovery (Own)"

picked_drives <- pbp_by_drive %>% 
  #filter(drive_result_detailed == "Punt") %>%
  select(game_id, drive_number)

ind <- sample(1:nrow(picked_drives), 20)
picked_drives <- apply(pbp_by_drive[ind, c("game_id", "drive_number")], 1, function(x) paste(x, collapse = " "))
picked_drives

dim(pbp_by_drive[ind, ])
# View(pbp_by_drive[ind, ])

j <- 3

View(pbp %>% 
       mutate(game_id_drive_number = paste(game_id, drive_number)) %>%
       filter(game_id_drive_number %in% picked_drives[j]) %>% .[, c("game_id", "pos_team", "def_pos_team", "pos_team_score", "def_pos_team_score", 
                                                                    "play_type", "play_text",
                                                                    "yards_gained",
                                                                    "yards_to_goal", "yards_to_goal_end",
                                                                    "drive_time_minutes_elapsed", "drive_time_seconds_elapsed",
...
View(pbp_by_drive %>% 
       mutate(game_id_drive_number = paste(game_id, drive_number)) %>%
       filter(game_id_drive_number %in% picked_drives[j]))
...
View(pbp %>% 
       filter(#play_type == "Fumble Recovery (Own)", 
         yards_gained != yds_rushed, !penalty_flag, !penalty_declined) %>%
       .[, c("game_id", "pos_team", "def_pos_team", "pos_team_score", "def_pos_team_score", 
             "play_type", "play_text", "penalty_flag", "yds_penalty", "fumble_vec",
             "yards_gained", "yds_rushed", "yds_receiving", 
             "drive_time_minutes_elapsed", "drive_time_seconds_elapsed",
             "period", "drive_time_minutes_end", "drive_time_seconds_end",
             "down")])

##    "yards_gained" on punts... do we do the "net_gain", hence PUNT/KICKOFF YARDS (before the return) go into NEGATIVE??? Then MANY POSSESSIONS after PUNTS/KICKOFFS will be NEGATIVE yards... and those OFF OF TURNOVERS would NOT BE...
##    "yards_gained" on Field goals => just adds the yardage of the field goal onto the drive length.. DON'T NEED THAT...
##    
##    "yards_gained" on kickoffs... do we do IMPREVEMENT OVER 25yards ("yds_kickoff_25yd_improvement")?? Or NET GAIN, "yds_kickoff_net" (with kick length going into NEGATIVE CREDIT)? Or just RETURN YARDS???
##    TD is officially recorded as 1st down
