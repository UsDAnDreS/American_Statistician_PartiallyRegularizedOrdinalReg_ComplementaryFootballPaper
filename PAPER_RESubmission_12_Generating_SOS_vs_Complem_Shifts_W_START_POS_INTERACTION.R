...
## CFB
## 
## Gets fed into "RESubmission_12p5_BOTH_CFB_AND_NFL_SOS_VS_COMP_SHIFTS_PLOT.R",
## to eventually obtain the ranking shift boxplots
#####

#######
## Calculating the full data fit, for plotting effects (Parts 5 & 6) and the ad-hoc diagnostics (Part 7)
#######
...
for (year in 2014:2020){
  ...
  projected.by.game[[as.character(year)]] <- pbp_by_drive.noNA %>%
...
save(projected.points.intercept, file= paste0("SOS_VS_COMP_ADJ_SHIFT_PLOTS_RESUBMISSION_CFB_W_START_POS_ORDNET_projected.points.intercept.RData"))
save(projected.by.game, file= paste0("SOS_VS_COMP_ADJ_SHIFT_PLOTS_RESUBMISSION_CFB_W_START_POS_ORDNET_projected.by.game.RData"))
# save(ordnet.fits, file=paste0("SOS_VS_COMP_ADJ_SHIFT_PLOTS_RESUBMISSION_CFB_W_START_POS_ORDNET_ordnet.fits.RData"))
...
##########
### GETTING THE OBJECTS FOR PLOTTING
##########

load(file= paste0("SOS_VS_COMP_ADJ_SHIFT_PLOTS_RESUBMISSION_CFB_W_START_POS_ORDNET_projected.points.intercept.RData"))
load(file=paste0("SOS_VS_COMP_ADJ_SHIFT_PLOTS_RESUBMISSION_CFB_W_START_POS_ORDNET_projected.by.game.RData"))
...
for (year in 2014:2020){
  my.df <- rbind(my.df,
                 data.frame(Year=year,
                      projected.points.intercept[[as.character(year)]]))
}
...
if (side == "Offense"){
  ...
  boot.df.complem <- my.df %>%
    mutate(Shift=Offense.Avg.Points.Avg.Takeaways - Offense.Avg.Points.With.Complem.But.As.Is)
  
} else {
  
  boot.df.complem <- my.df %>%
    mutate(Shift=Defense.Avg.Points.Avg.Takeaways - Defense.Avg.Points.With.Complem.But.As.Is)
}
...
if (side == "Offense"){
  ...
  boot.df.SOS <- my.df %>%
    mutate(Shift=Offense.Avg.Points.Avg.Takeaways - Offense.Avg.Points.Avg.Takeaways.But.SOS.As.Is)
  
} else {
  
  boot.df.SOS <- my.df %>%
    mutate(Shift=Defense.Avg.Points.Avg.Takeaways - Defense.Avg.Points.Avg.Takeaways.But.SOS.As.Is)
}


SeasonLong.Shifts.CFB.df <- list(SOS=boot.df.SOS, complem = boot.df.complem)
save(SeasonLong.Shifts.CFB.df, file=paste0("W_START_POS_INTERACTION_SeasonLong.Shifts.CFB.",side, ".df.Robj"))
...
for (year in 2014:2020){
  my.df <- rbind(my.df,
                 data.frame(Year=year,
                            projected.by.game[[as.character(year)]]))
}
...
SingleGame.Shifts.CFB.df <- list(SOS=boot.df.SOS, complem = boot.df.complem)
save(SingleGame.Shifts.CFB.df, file=paste0("W_START_POS_INTERACTION_SingleGame.Shifts.CFB.",side, ".df.Robj"))
